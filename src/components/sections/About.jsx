...
const About = () => {
  const intro = `An aspiring software engineer based in Singapore. As a self-taught engineer, 
                I have worked on some projects along the way. Stick around and have a look.`
  ...
  return (
    <>
      <div className="h-[120vh] md:max-lg:h-[120vh] lg:max-xl:h-screen xl:h-[80vh] w-screen bg-transparent relative 
      z-10 mt-[150px] dark:bg-black">
        <div className="h-full md:max-lg:w-screen lg:max-xl:w-[68%] xl:w-[72%] bg-transparent dark:bg-black inline-block 
        md:max-lg:inline-block lg:flex mx-auto relative before:content-[''] before:absolute before:bg-darkgray 
        before:left-[50%] before:bottom-[70px] before:translate-x-[-50%] before:w-[80%] before:h-0.5">
          ...
          relative transform transition-all duration-1000 ease-out ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-[100px] opacity-0'}`}>
            ...
            text-center md:max-lg:text-center lg:text-left lg:float-right leading-6 dark:text-gray">{intro}</p>
          </div>
        </div>
      </div>
    </> 
